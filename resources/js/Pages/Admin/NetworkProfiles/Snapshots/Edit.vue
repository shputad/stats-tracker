<template>
    ...
    <Head title="Edit Snapshot" />

    <AdminLayout>
        <div class="mx-auto max-w-3xl">
            <h1 class="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                <GoBack :href="route('admin.network-profiles.snapshots.index', profile.id)" class="mr-2" />
                Edit Snapshot for "{{ profile.account_id }}" – {{ profile.network_channel?.name || 'Unknown' }}
            </h1>
...
const submit = () => {
    form.put(route('admin.network-profiles.snapshots.update', {
