            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">
                        <GoBack :href="`/admin/network-profiles`" /> Balance Snapshots for "{{ profile.account_id }}" – {{ profile.network_channel?.name || 'Unknown' }}
                    </h1>
                ...
                <div class="flex items-center space-x-3 mt-4 sm:mt-0">
                    <Link :href="route('admin.network-profiles.snapshots.create', profile.id)"
                        ...
                        <tbody>
                            <tr v-for="snapshot in snapshotsGroup" :key="snapshot.id"
                                ...
                                <td class="p-3 text-center">
                                    <Link :href="route('admin.network-profiles.snapshots.edit', { network_profile: profile.id, snapshot: snapshot.id })"
...
const saveTopupValue = async (date) => {
    try {
        ...
        await router.put(route('admin.network-profiles.stats.updateTopup', {
            profile: profile.id,
            date: date,
        }), {
            topup_today: value,
...
const deleteSnapshot = async (id) => {
    ...
    if (result.isConfirmed) {
        await router.delete(route('admin.network-profiles.snapshots.destroy', {
